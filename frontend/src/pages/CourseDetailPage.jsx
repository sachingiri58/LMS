...
import { ArrowLeft, BookOpen, Clock, CodeSquare, Target, User, X,Play,ChevronDown, ArrowRight, Award,Sparkle, CheckCircle, Circle } from "lucide-react";
...
const CourseDetailPage = () => {
    ...
    //initiallize enrollment state based on course type
    useEffect(()=>{
      if(isCourseFree){
        setIsEnrolled(true);//auto be enroll for free courses
      }
      else{
        setIsEnrolled(false);
      }
...
//it will chcek for user is looged in or not 
//then it will check for is the course is free 
//then is will show the details of the lectures.
...
if(!course){
  ...
  return (
    <div className={courseDetailStyles.container}>
      ...
      <div className={`${courseDetailStyles.mainContainer} ${isPageLoaded ? courseDetailStyles.containerVisible : courseDetailStyles.containerHidden}`}>
...
<div className={courseDetailStyles.header}>
  <div className={courseDetailStyles.badge}>
   ...
   <span className={courseDetailStyles.badgeText}>
    {isCourseFree ? 'Free Courses' : 'premium Courses'}
   </span>
  </div>
...
<div className={`${courseDetailStyles.statsContainer} animate-delay-300`}>
...
<div className={`${courseDetailStyles.teacherStat} ${isTeacherAnimating ? courseDetailStyles.teacherAnimating:""}`}>
